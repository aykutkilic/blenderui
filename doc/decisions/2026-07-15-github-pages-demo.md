...
The repository's `example/` application is the primary visual showcase for
...
Deploy the existing `example/` application to:

<https://aykutkilic.github.io/flutterui/>

The deployment is owned by `.github/workflows/deploy-demo.yml`. It builds from
the `example/` directory with the repository path as Flutter's base href,
uploads the generated `example/build/web` directory as a Pages artifact, and
...
- `--base-href /flutterui/` is required because the site is hosted below the
