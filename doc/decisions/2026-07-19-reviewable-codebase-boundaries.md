...
A follow-up audit covered every Dart file under `lib/`, `example/lib/`,
`test/`, `example/test/`, and `tool/`, plus all maintained Markdown documents.
...
- `lib/` owns reusable presentation, editor chrome, interaction mechanics,
  ...
  controls.
- `example/` owns imitation data, tutorial prose, fake command outcomes,
...
- Completed plans are not kept as active backlogs. Decisions and dated outcomes
  belong in `doc/decisions/` and `doc/development-history.md`.

## Current example/library split
