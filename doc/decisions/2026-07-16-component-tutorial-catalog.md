...
Add `ComponentCatalogPage` as a standalone example entry point. It owns a
...
Keep the existing full Showcase workspace and its broader category workbench
intact. The direct `components_demo.dart` launcher is the focused documentation
entry point, while the showcase remains the realistic Blender application
composition.
...
- `flutter test test/component_catalog_test.dart` passes catalog browsing and
