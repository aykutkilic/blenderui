...
| `space_node.py` | Source-shaped Node Editor/Shader/Geometry/Compositor/Texture header with View/Select/Add/Node menus, tree context/data-block, pin, compositor backdrop/gizmo, snapping, overlay controls, and `BlenderNodeEditorSidebar` panel families | Partial |
...
| `space_sequencer.py` | Source-shaped Sequencer/Preview header with View/Select/Marker/Add/Strip/Image menus, view-mode, scene, overlap, snapping, display, channel, gizmo, and overlay controls, plus `BlenderSequencerSidebar` with Tool, Cache, Proxy, View, Safe Areas, Composition Guides, Annotation, and Strip families | Partial |
| `space_nla.py` | Source-shaped NLA header with View/Select/Marker/Add/Track/Strip menus, inline selected/hidden/missing/error filters, Filters and Snapping popovers, a separate playback footer, and `BlenderSequencerSidebar(nlaEditor: true)` with NLA Tool, View, Action, Transform, and Custom Properties families | Partial |
