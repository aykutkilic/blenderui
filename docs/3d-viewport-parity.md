...
Renderer-specific geometry stays in `examples/blenderui/lib/showcase_viewport.dart`.
...
- Running `flutter test` with `part of` registration files as independent test
  ...
  correct regression entry point is `examples/blenderui/test/widget_test.dart`.
