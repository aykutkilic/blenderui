...
- Split the former `example/` application into `examples/blenderui` and the
  standalone `examples/components` catalog.
- Moved the DAW host to `examples/daw` and kept reusable DAW behavior in
  `packages/blender_ui_daw`.
- Updated package paths, CI, structural checks, tests, and onboarding docs so
  each example can be run independently.

## 2026-07-22 — Replaced DAW device and plug-in fixtures with native discovery

- Added reusable native audio and MIDI adapters to `blender_ui_daw`.
- Added recursive VST3 bundle discovery and AudioToolbox Audio Unit discovery
  in the macOS runner; real Audio Units can be instantiated, automated, saved,
  and restored through the existing plug-in-host API.
- Added CoreAudio device/channel enumeration and native input/output-device and
  buffer selection, plus CoreMIDI endpoint discovery in Preferences.
- Added plug-in capability metadata so discovered VST3 bundles are clearly
  disabled until the isolated VST3 SDK host exists instead of failing as if
  they were loadable.
- Moved installed plug-in scanning off the main thread and retained in-memory
  catalogs only for tests and non-macOS platforms.
- Documented the deliberate example-runner sandbox exception and the remaining
  DSP/VST3 isolation work in the
  [decision record](decisions/2026-07-22-native-audio-plugin-and-midi-discovery.md).

## 2026-07-21 — Added switchable DAW areas and non-destructive audio editing

- Made every DAW dock area user-switchable from its Blender-style header and
  retained each area's editor type in the shared editor-session service.
- Added independent time zoom, draggable track heights, trim/stretch modes,
  clip offsets, source tempo/playback rate, clip and project loop controls.
- Added expandable arrangement automation lanes with point creation,
  selection, and dragging against the same immutable project state as the
  standalone automation editor.
- Added scale-aware piano-roll filtering and pitch snapping while preserving
  note creation, movement, resizing, timing snap, and velocity rendering.
- Added reusable audio-device discovery/reconfiguration Preferences, a
  horizontal track/master effect rack, and a geometry-node-style audio routing
  editor synchronized with the canonical effect chain.
- Added portable master-chain persistence and focused coverage for clip
  stretching, lanes, loops, scale filters, device switching, effect ordering,
  codec round trips, and editor switching.
- Fixed editor-area controllers notifying their session during dock layout,
  and documented nested Flutter package test resolution. See the
  [decision record](decisions/2026-07-21-daw-area-editing-and-audio-graphs.md).

## 2026-07-21 — Implemented the reusable DAW extension and example

- Added the `blender_ui_daw` extension package with immutable song, track,
  clip, MIDI, waveform, automation, mixer, transport, and plug-in host models.
- Added reusable arrangement, piano-roll, wave, automation, mixer, transport,
  plug-in browser, and plug-in rack surfaces composed from BlenderUI controls.
- Added a dedicated multi-workspace DAW example that reuses BlenderUI docking,
  history, commands, live keymaps, status, and Preferences services.
- Added clip move/resize/split/duplicate, MIDI note create/move/resize/delete,
  automation point create/move/delete and Read/Write/Touch/Latch state, audio
  normalization/reverse/fades, mixer controls, loop/tempo, and undo/redo.
- Added a versioned portable project codec, host-neutral save/load/autosave,
  real-time audio-engine contract, deterministic test engine, and native
  VST3/Audio Unit/CLAP method-channel adapter with contract coverage.
- Connected the DAW runner's native macOS `Preferences…` menu action to Flutter
  and moved immediate frame scheduling into `BlenderPreferencesService`, fixing
  the case where `Cmd+,` appeared to work only after another UI repaint.
- Corrected `BlenderDropdown` sizing in unbounded scrolling headers, found by
  the DAW boot regression, while retaining fill behavior in bounded forms.
- Verified the native menu with `ibtool`, added a direct lifecycle-channel
  widget test, and built the debug macOS application. See the
  [decision record](decisions/2026-07-21-daw-extension-and-native-preferences.md).
...
- Added a 2x render-level regression that samples the selected pixels at the
  ...
  example catalog suite must be invoked from `examples/blenderui/`; running it from the
...
- Added a live Components tutorial catalog and interactive examples, keeping
  application-specific demonstrations in `examples/blenderui/` while public primitives
...
- Rebuilt the effect chain with precise insertion targets, instantiate-first
  project commits, synchronized unload, compact bypass controls, engine-backed
  VU meters, and direct hosted-parameter editing.
- Added Auto Filter, EQ Eight, Compressor, Dynamics Compressor, Delay, and
  Reverb as a shared library catalog; the macOS runner instantiates real Apple
  Audio Unit processors instead of sample plug-in names.
- Kept discovery-only VST3 bundles visibly unavailable until an isolated SDK
